...
const LoginPage = () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (response.ok) {
                    ...
                    console.log('Inicio de sesión exitoso:', data);
                    router.push(data.isAdmin ? '/backoffice' : '/dashboard');
                }
                else {
    ...
    return (<div style={{ minHeight: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center', backgroundColor: '#f0f0f0' }}>
      <div style={{ backgroundColor: 'white', padding: '2rem', borderRadius: '0.5rem', boxShadow: '0 4px 6px rgba(0, 0, 0, 0.1)', maxWidth: '24rem', width: '100%' }}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="m@ejemplo.com" required className="w-full p-2 border rounded"/>
          </div>
          <div>
